...
import { DataInfo, DataInfoResponse, defaultDataData } from "@/app/api/teacher/userinfo/utils";
...
const Search = () => {
  ...
  const [selectedUser, setSelectedUser] = React.useState<DataInfo>(defaultDataData);
  const [userList, setUserList] = React.useState<DataInfo[]>([]);
  ...
  const searchUser = async () => {
    setLoading(true);
    try{
      const res: AxiosResponse<DataInfoResponse> = await instance.post(
  ...
  React.useEffect(() => {
    setSelectedUser(defaultDataData);
  ...
  const putJasupAdminData = async (id: string) => {
    setLoading(true);
    const loading = alert.loading("선생님 권한 부여 중입니다.");
    try{
      const res = await instance.put("/api/teacher/teachers", {
        id
      });
      router.refresh();
  ...
  return (
    <article className={[
      ...
      <section className="flex flex-row justify-center items-center w-full gap-4">
        <input 
          type="text" 
          placeholder="검색할 사용자 이름을 입력해주세요." 
